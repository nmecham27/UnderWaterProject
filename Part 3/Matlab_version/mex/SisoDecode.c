				 ...
				 const mxArray *prhs[] )
{
	...
	if (nrhs < 3 ) {
		...
		//mexPrintf("DataLength %d CodeLength %d \n", DataLength, CodeLength);	
	...
	/*for (j=DataLength-1;j<DataLength;j++)
	{
		mexPrintf("   input_u_float[%d] = %f  input_c_float[%d] = %f, %f\n", j, input_u_float[j], j, input_c_float[j], input_c_float[j+10] ); 
	}*/
	mexPrintf("SISO: %d, %d, %d, %d, %d, %d, %d, %d \n",out1[0], out1[1], out1[2], out1[3], out0[0], out0[1], out0[2], out0[3]);
