...
int GFq_m(int a, int b, int q)
{
  ...
  printf("GFq_m: I'm afraid I don't know how to multiply in GFq\n");
  return 0 ;
}
...
int GFq_inv(int a, int q)
{
  ...
  if ( a == 0) printf("GFq_inv: no inverse for 0!\n");; 
  if ( a == 1 )  return 1 ;
  switch (q){
  case 256:
  ...
  printf("GFq_inv: not defined inverse for  GFq\n");
  return 0 ;
}
...
void permute(double* input, double* buffer, int shift, int Qary)
{
	...
	for(q=0;q<Qary;q++)
	{
		index = GFq_m(q, shift, Qary);
//		printf("index  q %2d  %2d\n", index, q);
